...
from nnet.utils.spatial_regularization import SpatialRegularizationConfig, compute_spatial_regularization
...
class TWISTER(models.Model):
    ...
    def __init__(self, env_name, override_config={}, name="Transformer-based World model wIth contraSTivE Representations (TWISTER)"):
        ...
        # World Model Params
        requested_model_size = str(override_config.get("model_size", self.config.model_size)).lower()
        if requested_model_size not in model_size_aliases:
            raise ValueError(
                "Unknown model_size {!r}. Available sizes: {}".format(
                    override_config.get("model_size", self.config.model_size),
                    ", ".join(sorted(model_sizes.keys())),
                )
            )
        self.config.model_size = model_size_aliases[requested_model_size]
        ...
        self.config.wm_initial_source = "real"
        self.config.freeze_wm_after_step = -1
        self.config.freeze_wm_original_step = 100000
        ...
        self.config.loss_spatial_regu_scale = 0.0
        self.config.spatial_regu_enabled = False
        self.config.spatial_regu_mask_weights = {"mask1": 1.0, "mask2": 0.0, "mask3": 0.0}
        ...
        self.config.fast_contrastive_augments = True
    ...
    def load(self, path, load_optimizer=True, verbose=True, strict=True):
        ...
        if load_optimizer and checkpoint["optimizer_state_dict"] is not None:
            ...
            # Overall Dyna training step. Optimizer-specific steps are already
            # restored from their optimizer states above.
    ...
    def compile(self):
        ...
        # Overall Dyna training step. The world-model optimizer step stops
        # advancing after WM freeze, but eval/log/checkpoints must keep
        # following policy-training progress.
    ...
    def train_step(self, inputs, targets, precision, grad_scaler, accumulated_steps, acc_step, eval_training):
        ...
        # World Model Step
        freeze_after = int(getattr(self.config, "freeze_wm_after_step", -1))
        dyna_step = int(self.model_step.item())
        wm_frozen = freeze_after >= 0 and dyna_step >= freeze_after
        freeze_original = int(getattr(self.config, "freeze_wm_original_step", self.config.epochs * self.config.epoch_length))
        batch_metrics["dyna/wm_frozen"] = torch.tensor(float(wm_frozen), device=self.device, dtype=torch.float64)
        batch_metrics["dyna/freeze_wm_after_step"] = torch.tensor(float(freeze_after), device=self.device, dtype=torch.float64)
        batch_metrics["dyna/freeze_progress"] = torch.tensor(float(dyna_step) / float(freeze_original), device=self.device, dtype=torch.float64)
        if not wm_frozen:
            ...
            self.set_require_grad([self.encoder_network, self.decoder_network, self.rssm, self.reward_network, self.continue_network, self.contrastive_network], False)
            world_model_training = self.world_model.training
            self.world_model.eval()
            with torch.no_grad():
                _, _, _, _ = self.world_model.forward_model(inputs, targets, compute_metrics=False)
            self.world_model.train(mode=world_model_training)
    ...
    class WorldModel(models.Model):
        ...
        def forward(self, inputs):
            ...
            # Unpack Inputs 
            if len(inputs) == 7:
                states, actions, rewards, dones, is_firsts, model_steps, masks = inputs
            else:
            ...
            if self.config.loss_contrastive_scale != 0.0 and self.config.contrastive_steps > 0:

                # Flatten B and L to ensure diff augment for each sample (B*L, 3, H, W)
                states_flatten = states.flatten(0, 1)

                # Augment
                states_aug = self.apply_contrastive_augments(states_flatten).reshape(states.shape)
            ...
            if self.config.spatial_regu_enabled:
                spatial_config = SpatialRegularizationConfig(
                    enabled=True,
                    weight=float(self.config.loss_spatial_regu_scale),
                    mask_weights=dict(self.config.spatial_regu_mask_weights),
                    image_channels=self.config.image_channels,
                )
                spatial_loss, spatial_metrics = compute_spatial_regularization(
                    pred=states_pred.mode(),
                    target=states.detach(),
                    masks=masks,
                    config=spatial_config,
                    return_metrics=True,
                )
                self.add_loss("spatial_regu", spatial_loss, weight=1.0)
                for key, value in spatial_metrics.items():
                    self.add_metric(key, value.detach())
    ...
    class ActorModel(models.Model):
        ...
        def forward(self, inputs):
            ...
            wm_rewards = model_rewards.mode()[:, 1:]
            returns = self.compute_td_lambda(rewards=wm_rewards, values=values.mode()[:, 1:], discounts=self.config.gamma * discounts[:, 1:])
            wm_rollout_returns = wm_rewards.sum(dim=1)
            self.add_info("wm_rollout_reward_mean", wm_rewards.mean().item())
            self.add_info("wm_rollout_return_mean", wm_rollout_returns.mean().item())
            self.add_info("wm_rollout_return_std", wm_rollout_returns.std(unbiased=False).item())
            self.add_info("wm_rollout_return_min", wm_rollout_returns.min().item())
            self.add_info("wm_rollout_return_max", wm_rollout_returns.max().item())
    ...
    def compute_contrastive_loss(self, features_x, features_y):
        ...
        acc_con = (features.argmax(dim=-1) == torch.arange(features.shape[0], device=features.device)).float().mean()

        return info_nce_loss, acc_con

    def apply_contrastive_augments(self, states_flatten):
        if not self.config.fast_contrastive_augments:
            return torch.stack(
                [self.config.contrastive_augments(states_flatten[b]) for b in range(states_flatten.shape[0])],
                dim=0,
            )

        transform = self.config.contrastive_augments
        size = tuple(transform.size) if isinstance(transform.size, (list, tuple)) else (transform.size, transform.size)
        scale = tuple(transform.scale)
        ratio = tuple(transform.ratio)
        return self.random_resized_crop_batch(states_flatten, size=size, scale=scale, ratio=ratio)

    def random_resized_crop_batch(self, images, size, scale, ratio, max_attempts=10):
        ...
        min_log_ratio = math.log(ratio[0])
        max_log_ratio = math.log(ratio[1])
        target_area = torch.empty((batch, max_attempts), device=device, dtype=dtype).uniform_(scale[0], scale[1]) * area
        aspect_ratio = torch.exp(
            torch.empty((batch, max_attempts), device=device, dtype=dtype).uniform_(min_log_ratio, max_log_ratio)
        )

        crop_w_all = torch.round(torch.sqrt(target_area * aspect_ratio)).to(torch.long)
        crop_h_all = torch.round(torch.sqrt(target_area / aspect_ratio)).to(torch.long)
        valid = (crop_w_all > 0) & (crop_w_all <= width) & (crop_h_all > 0) & (crop_h_all <= height)
        has_valid = valid.any(dim=1)
        first_valid = valid.to(torch.long).argmax(dim=1)

        batch_indices = torch.arange(batch, device=device)
        crop_h = crop_h_all[batch_indices, first_valid]
        crop_w = crop_w_all[batch_indices, first_valid]

        min_ratio, max_ratio = ratio
        in_ratio = width / height
        if in_ratio < min_ratio:
            fallback_w = width
            fallback_h = int(round(fallback_w / min_ratio))
        elif in_ratio > max_ratio:
            fallback_h = height
            fallback_w = int(round(fallback_h * max_ratio))
        else:
            fallback_h = height
            fallback_w = width

        fallback_h = torch.full((batch,), fallback_h, device=device, dtype=torch.long)
        fallback_w = torch.full((batch,), fallback_w, device=device, dtype=torch.long)
        crop_h = torch.where(has_valid, crop_h, fallback_h).clamp(1, height)
        crop_w = torch.where(has_valid, crop_w, fallback_w).clamp(1, width)

        top = torch.floor(torch.rand(batch, device=device) * (height - crop_h + 1).to(dtype)).to(torch.long)
        left = torch.floor(torch.rand(batch, device=device) * (width - crop_w + 1).to(dtype)).to(torch.long)

        ys = torch.arange(out_h, device=device, dtype=dtype)
        xs = torch.arange(out_w, device=device, dtype=dtype)
        y_src = top[:, None, None].to(dtype) + (ys[None, :, None] + 0.5) * crop_h[:, None, None].to(dtype) / out_h - 0.5
        x_src = left[:, None, None].to(dtype) + (xs[None, None, :] + 0.5) * crop_w[:, None, None].to(dtype) / out_w - 0.5

        y_grid = (y_src + 0.5) / height * 2.0 - 1.0
        x_grid = (x_src + 0.5) / width * 2.0 - 1.0
        grid = torch.stack(
            [
                x_grid.expand(batch, out_h, out_w),
                y_grid.expand(batch, out_h, out_w),
            ],
            dim=-1,
        )
    ...
    def eval_step(self, inputs, targets, verbose=False):
        ...
        episode_outputs = [
            ...
            for _ in range(int(self.config.eval_episodes))
        ]
        scores = torch.stack([
            torch.as_tensor(output.score, device=self.device, dtype=torch.float32)
            for output in episode_outputs
        ])
        lengths = torch.stack([
            torch.as_tensor(output.steps, device=self.device, dtype=torch.float32)
            for output in episode_outputs
        ])
        outputs = {
            "score": scores.mean(),
            "steps": lengths.mean(),
            "eval_real/score_mean": scores.mean(),
            "eval_real/score_std": scores.std(unbiased=False),
            "eval_real/score_min": scores.min(),
            "eval_real/score_max": scores.max(),
            "eval_real/length_mean": lengths.mean(),
            "eval_real/episodes": torch.tensor(int(self.config.eval_episodes), device=self.device),
        }
    ...
    def log_figure(self, step, inputs, targets, writer, tag, save_image=False): 
        ...
        with torch.no_grad():
            ...
            if self.config.loss_contrastive_scale != 0.0 and self.config.contrastive_steps > 0:
                ...
                # Augment
                states_aug_con = self.apply_contrastive_augments(states_flatten).reshape(states.shape)
