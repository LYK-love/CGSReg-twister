...
class Model(modules.Module):
    ...
    def log_step(self, losses, metrics, infos, writer, step, tag):
        def log_path(key):
            if key.startswith("eval_real/") or key.startswith("dyna/"):
                return key
            return os.path.join(tag, key)

        # Losses
        for key, value in losses.items():
            writer.add_scalar(log_path(key), value, step)
        ...
        for key, value in infos.items():
            if isinstance(value, float) or isinstance(value, int):
                writer.add_scalar(log_path(key), float(value), step)
        ...
        if callback_path is not None and wandb_logging:
            try:
                wandb.init(
                    ...
                    name=os.environ.get("WANDB_NAME", os.path.basename(os.path.normpath(callback_path))),
                    id=os.environ.get("WANDB_RUN_ID") or None,
                    resume=os.environ.get("WANDB_RESUME", "allow") if os.environ.get("WANDB_RUN_ID") else None,
                    dir=os.path.join(callback_path, "wandb"),
                    config={
                        "eval_real_episodes": int(getattr(self.config, "eval_episodes", 0)),
                        "eval_real_every_steps": None if eval_period_step is None else int(eval_period_step),
                        "eval_real_policy": "greedy",
                        "eval_real_env": os.environ.get("env_name", "real_env"),
                    },
                )
