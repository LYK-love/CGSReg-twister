...
class TSSM(nn.Module):
    ...
    def __init__(
        ...
        self.action_mixer = modules.MultiLayerPerceptron(
            dim_input=(
                self.stoch_size * self.discrete + self.num_actions
                if self.discrete
                else self.stoch_size + self.num_actions
            ),
        ...
        self.dynamics_predictor = modules.Linear(
            ...
            out_features=(
                self.discrete * self.stoch_size
                if self.discrete
                else 2 * self.stoch_size
            ),
    ...
    def initial(
        ...
        initial_state = structs.AttrDict(
            ...
            deter=torch.zeros(
                batch_size, seq_length, self.hidden_size, dtype=dtype, device=device
            ),
            hidden=None,
        )
    ...
    def observe(
        ...
        prev_states = {
            key: torch.cat([prev_state[key], value], dim=1)
    ...
    def imagine(
        ...
        for h in range(img_steps):
            ...
            if is_firsts_hidden is not None:
                ...
                is_firts_mask = modules.return_is_firsts_mask(
                    is_firsts=is_firsts, is_firsts_hidden=is_firsts_hidden
                )
                mask = mask.minimum(is_firts_mask)

                # Concat is_firsts to hidden is_firsts (B, C)
                is_firsts_hidden = torch.cat(
                    [is_firsts_hidden[:, 1:], is_firsts], dim=1
                )
        ...
        # Stack Lists
        img_states = {
            k: torch.concat(v, dim=1) for k, v in img_states.items()
        }  # (B, 1+img_steps, D)
    ...
    def get_feat(self, state, blocks_deter_id=None):
        ...
        return torch.cat(
            [
                state["stoch"].flatten(start_dim=-2, end_dim=-1),
                (
                    state["deter"]
                    if blocks_deter_id is None
                    else state["blocks_deter"][blocks_deter_id]
                ),
            ],
            dim=-1,
        )
    ...
    def get_dist(self, state):
        ...
        return torch.distributions.Independent(
            distributions.OneHotDist(
                logits=state["logits"], uniform_mix=self.uniform_mix
            ),
            1,
        )
    ...
    def slice_hidden(self, hidden):
        ...
        hidden = [
            (
                hidden_blk[0][:, -self.att_context_left :],
                hidden_blk[1][:, -self.att_context_left :],
            )
            for hidden_blk in hidden
        ]
    ...
    def forward_img(
        ...
        # Recurrent
        assert (
            self.get_hidden_len(prev_states["hidden"]) <= self.att_context_left
    ...
    def forward(
        ...
        if is_firsts.any():
            ...
            init_state = self.initial(
                batch_size=prev_actions.shape[0],
                seq_length=prev_actions.shape[1],
                dtype=prev_actions.dtype,
                device=prev_actions.device,
            )
            for key, value in prev_states.items():
                ...
                # Reset first States
                    ...
                    is_firsts_r = torch.reshape(
                        is_firsts,
                        is_firsts.shape
                        + (1,) * (len(value.shape) - len(is_firsts.shape)),
                    )
                    prev_states[key] = (
                        value * (1.0 - is_firsts_r) + init_state[key] * is_firsts_r
                    )

            # Mask positions of past trajectories # (B, 1, L, Th+L)
            is_firts_mask = modules.return_is_firsts_mask(
                is_firsts.squeeze(dim=-1), is_firsts_hidden=is_firsts_hidden
            )
            # print(mask.shape, is_firts_mask.shape, is_firsts.shape)
            mask = mask.minimum(is_firts_mask)

        # Forward Img
        prior = self.forward_img(
            prev_states,
            prev_actions,
            mask,
            return_att_w=return_att_w,
